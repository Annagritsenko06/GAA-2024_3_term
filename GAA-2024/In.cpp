		...
		for (int i = 0; i < textSize; i++)		// заполнение
		{
			...
			case IN::Y:		// строковый литерал
			{
				...
				*buffer = IN_CODE_NULL;
				...
				for (int j = i + 1; text[j] != IN_CODE_SINGLE_QUOTE; j++)		// если литерал не закрыт - перевод строки будет раньше кавычки
				{
					if (code[text[j]] == IN::N)
					{
						Log::writeError(stream, Error::GetError(311, line, 0));
				...
				if (isltrlgood)
				{
					buffer[bufpos++] = IN_CODE_SINGLE_QUOTE;
					for (int j = 1; ; j++)
					{
						if (j == 256 || i + j == textSize)
						{
						...
						buffer[bufpos++] = text[i + j];
						if (text[i + j] == IN_CODE_SINGLE_QUOTE)	// закрывающа€ кавычка
						{
							buffer[bufpos] = IN_CODE_NULL;
							addWord(words, buffer, line);
							i = i + j;
