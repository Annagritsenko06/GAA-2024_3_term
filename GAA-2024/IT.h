...
#define NUM_DEFAULT	0x00000000				// значение по умолчанию для bint
#define STR_DEFAULT	0x00					// значение по умолчанию для sting
#define BOOL_DEFAULT	false               // значение по умолчанию для tbool
#define SMBL_DEFAULT  '\0'						// значение по умолчанию для osymb
...
#define SMBL_MAXSIZE 255	                // максимальная длина символьного литерала
...
namespace IT
{
	enum IDDATATYPE { NUM = 1, STR = 2,SMBL = 3,TBOOL = 4, PROC = 5, UNDEF=6 };	// типы данных идентификаторов: числовой, строковый, без типа, неопределенный
	...
	struct Entry
	{
		union
		{
			...
			struct
			{
				int smbl_len;					// количество символов
				char smbl[SMBL_MAXSIZE - 1];	// символы
			} vsmbl;
			struct
			{
		...
		Entry()								// конструктор без параметров
		{
			...
			this->value.vsmbl.smbl_len = NULL;
		};
