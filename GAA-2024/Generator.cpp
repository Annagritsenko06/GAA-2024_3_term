...
namespace Gener		// генерация кода в assembler
{
		...
		while (LEXEMA(j) != LEX_RIGHTTHESIS) // пока параметры не кончатся
		{
			if (LEXEMA(j) == LEX_ID) // параметр
			{
				...
				case IT::IDDATATYPE::NUM:
					str += string(paramEntry.id) + " : sdword, ";
					break;
				case IT::IDDATATYPE::STR:
					str += string(paramEntry.id) + " : dword, "; // Для строкового типа
					break;
				case IT::IDDATATYPE::TBOOL:
					str += string(paramEntry.id) + " : db, "; // Для булевого типа
					break;
				case IT::IDDATATYPE::SMBL:
					str += string(paramEntry.id) + " : db, "; // Для символьного типа
					break;
				default:
					str += string(paramEntry.id) + " : dword, "; // Для других типов
					break;
				}
			}
			j++;
	...
	string genConditionCode(Lexer::LEX& tables, int i, string& cyclecode) // оператор условия 
	{
		...
		case GREATER_EQUAL:     rstr = "jge"; wstr = "jl";  break; 
		case LESS_EQUAL:        rstr = "jle"; wstr = "jg";  break; 
		}
		...
		if (c)
		{
			str = str + "\n" + rstr + " cycle" + itoS(conditionnum); // генерация кода цикла 
			cyclecode = str;
	...
	vector <string> startFillVector(Lexer::LEX& tables)	// заполнение вектора
	{
		...
		for (int i = 0; i < tables.idtable.size; i++)	// const, data
		{
			...
			if (tables.idtable.table[i].idtype == IT::IDTYPE::L)		// литерал - в .const
			{
				...
				case IT::IDDATATYPE::TBOOL: // Добавление обработки для булевского типа
					str = str + " db " + (e.value.vbool ? "0" : "1") + "; Булевый литерал"; // 1 для true, 0 для false
					break;
				case IT::IDDATATYPE::SMBL: // Обработка символьного типа
					str += " db '" + string(e.value.vsmbl.smbl) + "', 0; Символьный литерал";
					break;
				}
				vlt.push_back(str);
			}
			else if (tables.idtable.table[i].idtype == IT::IDTYPE::V)	// переменная - в .data
			{
				...
				case IT::IDDATATYPE::TBOOL: // Добавление обработки для булевских переменных
					str = str + " db ?; Булевый тип"; // Инициализация булевой переменной
					break;
				case IT::IDDATATYPE::SMBL: // Обработка символьных переменных
					str += " db ?; Символьная переменная";
					break;
